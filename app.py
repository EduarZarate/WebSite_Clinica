...
app.config['MYSQL_PASSWORD'] = '140801'
...
def sacar_citas():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("SELECT id FROM medicos WHERE especialidad=%s", [medico])
...
def Medicos():
    cur = mysql.connection.cursor()
    cur.execute('SELECT * FROM farmacia.medicos')
...
def pacientes():
    cur = mysql.connection.cursor()
    cur.execute('SELECT * FROM farmacia.pacientes')
